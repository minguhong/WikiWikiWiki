...
- `templates/base.php` を上書きする場合、`<head>` 内の `/rss.xml`, `/atom.xml`, `/feed.json` 用 feed autodiscovery `<link>` を維持してください。
- `templates/edit.php` を上書きする場合、`csrf_token`, `original_revision`, `delete_history` の hidden input を必ず維持してください。
- `original_revision` が欠落すると、保存リクエストは契約上 fail-closed になります。
