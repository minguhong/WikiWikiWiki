...
### Required contract when overriding templates

- If you override `templates/base.php`, keep feed autodiscovery `<link>` tags for `/rss.xml`, `/atom.xml`, and `/feed.json` inside `<head>`.
- If you override `templates/edit.php`, keep hidden inputs named `csrf_token`, `original_revision`, and `delete_history`.
- Missing `original_revision` causes save requests to fail by contract.
