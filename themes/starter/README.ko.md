...
- `templates/base.php`를 오버라이드하면 `<head>` 안에 `/rss.xml`, `/atom.xml`, `/feed.json` feed autodiscovery `<link>`를 유지해야 합니다.
- `templates/edit.php`를 오버라이드하면 `csrf_token`, `original_revision`, `delete_history` hidden input을 반드시 유지해야 합니다.
- `original_revision`이 없으면 저장 요청은 계약상 실패-폐쇄됩니다.
