...
- Route/API behavior:
  ...
  - `wikiwikiwiki/src/handlers/media.php`
...
- Repo-maintainer only (not in release bundle): `./scripts/update-preflight.sh`
- Corpus render audit (requires live `content/` directory): `php scripts/markdown-audit.php`
- Boot smoke:
...
| --- | --- |
| Route/API (`router.php`, `handlers/api.php`, `handlers/feed.php`, `handlers/media.php`) | `php -l wikiwikiwiki/src/router.php` ; `php -l wikiwikiwiki/src/handlers/api.php` ; `php -l wikiwikiwiki/src/handlers/feed.php` ; `php -l wikiwikiwiki/src/handlers/media.php` ; boot smoke from section 16 ; feed smoke (`/rss.xml`, `/atom.xml`, `/feed.json`) ; media smoke (`/favicon.svg`, `/og-image.png`) |
...
| i18n only | JSON validity and key parity for `ko.json`/`en.json` plus one representative page render |
