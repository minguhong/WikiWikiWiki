...
<head>
  ...
  <meta property="og:image" content="<?= html(absolute_asset('images/og-image.jpg')) ?>">
  <meta property="og:image:type" content="image/jpeg">
  <meta property="og:locale" content="<?= html($languageCode) ?>">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:image" content="<?= html(absolute_asset('images/og-image.jpg')) ?>">
  <link rel="canonical" href="<?= html($ogUrl) ?>">
  <link rel="alternate" type="application/rss+xml" title="<?= html($wikiTitle . ' RSS') ?>" href="<?= html(url('/rss.xml')) ?>">
  <link rel="alternate" type="application/atom+xml" title="<?= html($wikiTitle . ' Atom') ?>" href="<?= html(url('/atom.xml')) ?>">
  <link rel="alternate" type="application/feed+json" title="<?= html($wikiTitle . ' JSON Feed') ?>" href="<?= html(url('/feed.json')) ?>">
  <link rel="icon" type="image/png" href="<?= html(asset('images/favicon.png')) ?>">
  <?= css('css/style.css') ?>
</head>
<body id="<?= html($template) ?>">
  ...
  <div class="document">
    <header class="header" aria-label="<?= t('aria.site_header') ?>">
      <h1 class="header-title"><a href="<?= html(url()) ?>"><?= html($wikiTitle) ?></a></h1>
      <nav class="header-nav" aria-label="<?= t('aria.main_navigation') ?>">
        <a href="<?= html(url('/search')) ?>" data-shortcut="search" aria-label="<?= t('nav.search') ?>"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
