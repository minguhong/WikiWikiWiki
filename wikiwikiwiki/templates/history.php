<section class="section">
  ...
  <div class="section-main">
            ...
            <tbody>
              <?php foreach ($versions as $version): ?>
                <tr>
                  ...
                  <?php else: ?>
                    <td>
                      <a href="<?= html(url($page, '/history/' . $version['timestamp'])) ?>"><?= html($version['date']) ?></a>
                    </td>
                    <td>
                      <?= $version['author'] !== '' ? html($version['author']) : t('history.unknown_editor') ?>
                    </td>
                  <?php endif ?>
                </tr>
              <?php endforeach ?>
    ...
    <?php else: ?>
      <h2 class="table-title">
        ...
        <span><?= $author !== '' ? html($author) : t('history.unknown_editor') ?></span>
      </h2>
      <?php if ($diffTooLarge): ?>
        <div class="content">
  ...
  <footer class="section-footer">
    ...
    <?php else: ?>
      <div class="buttons">
        <div>
          <a class="button" href="<?= html(url($page, '/history')) ?>"><?= t('button.back') ?></a>
        </div>
        <div>
          <?php if ($olderTimestamp): ?>
            <a class="button" href="<?= html(url($page, '/history/' . $olderTimestamp)) ?>"><?= t('button.prev') ?></a>
          <?php else: ?>
            <button class="button" type="button" disabled><?= t('button.prev') ?></button>
          <?php endif ?>
          <?php if ($newerTimestamp): ?>
            <a class="button" href="<?= html(url($page, '/history/' . $newerTimestamp)) ?>"><?= t('button.next') ?></a>
          <?php else: ?>
            <button class="button" type="button" disabled><?= t('button.next') ?></button>
          <?php endif ?>
          <?php if (is_admin()): ?>
            <form action="<?= html(url($page, '/history/' . $timestamp)) ?>" method="post">
          ...
          <?php endif ?>
          <?php if (is_admin() && ($canRestore ?? $newerTimestamp !== null)): ?>
            <form action="<?= html(url($page, '/history/' . $timestamp)) ?>" method="post">
              <input type="hidden" name="csrf_token" value="<?= html($csrfToken) ?>">
