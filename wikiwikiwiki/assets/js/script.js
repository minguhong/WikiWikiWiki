...
function initEditMode() {
  ...
  let originalTitleValue = titleInput instanceof HTMLInputElement ? titleInput.value : '';
  ...
  const isEditorDirty = () => {
    if (textarea.value !== originalValue) return true;
    if (titleInput instanceof HTMLInputElement && titleInput.value !== originalTitleValue) return true;
    return false;
  };
  ...
  const markEditorClean = () => {
    ...
    if (titleInput instanceof HTMLInputElement) {
      originalTitleValue = titleInput.value;
    }
  ...
  const syncDirtyStateFromInput = () => {
    syncBeforeUnloadListener();
    if (isSubmitting) {
      resetSubmitState();
    }
  };
  textarea.addEventListener('input', syncDirtyStateFromInput);
  if (titleInput instanceof HTMLInputElement) {
    titleInput.addEventListener('input', syncDirtyStateFromInput);
  }
