...
function handle_login(string $method, array $matches): void
{
    ...
    if ($method === 'POST') {
        ...
        $rememberLoginChecked = request_string($_POST, 'remember_login') === '1';

        $loginResult = login($inputUsername, request_string($_POST, 'password'));

        if ($loginResult === true) {
            if (!remember_login_revoke_current()) {
                wiki_log('auth.remember_login_revoke_failed_after_login', ['username' => current_user()], 'warning');
                if (!$rememberLoginChecked) {
                    remember_login_clear_cookie();
                }
            }
            $loggedInUser = current_user();
            if ($rememberLoginChecked && is_string($loggedInUser) && $loggedInUser !== '' && !remember_login_issue($loggedInUser)) {
                wiki_log('auth.remember_login_issue_failed', ['username' => $loggedInUser], 'warning');
            }
            redirect($page ?: url('/'));
        }
...
function handle_install(string $method, array $matches): void
{
    ...
    if ($method === 'POST') {
        ...
        $hadConfigFile = is_file($configPath);
        $existingConfigRaw = $hadConfigFile ? @file_get_contents($configPath) : false;
        if ($hadConfigFile && !is_string($existingConfigRaw)) {
        ...
        $restoreConfigSnapshot = static function () use ($configPath, $existingConfigRaw, $hadConfigFile): void {
            wiki_with_lock(static function () use ($configPath, $existingConfigRaw, $hadConfigFile): void {
                if ($hadConfigFile) {
                    if (!is_string($existingConfigRaw) || !file_put_atomic($configPath, $existingConfigRaw)) {
                        wiki_log('install.config_restore_failed', ['path' => $configPath], 'error');
                        return;
                    }
                ...
                if (is_file($configPath)) {
                    if (!@unlink($configPath)) {
                        clearstatcache(true, $configPath);
                        if (is_file($configPath)) {
                            wiki_log('install.config_remove_failed', ['path' => $configPath], 'error');
                        }
                    }
                }
                clearstatcache(true, $configPath);
...
function install_config_writable(string $configPath): bool
{
    ...
    return is_dir(BASE_DIR) && is_writable(BASE_DIR);
}
...
function install_selected_language(): string
{
    ...
    return $allowed[0] ?? (function_exists('default_language_code') ? default_language_code() : 'en');
}
...
function handle_register(string $method, array $matches): void
{
    ...
    if ($method === 'POST') {
        ...
        $registerRateKey = register_rate_limit_key($inputUsername);
...
function auth_account_rate_limit_key(string $username, string $action): string
{
    return 'account:' . client_ip() . ':' . user_normalize_username($username) . ':' . $action;
}

function auth_account_rate_limited(string $username, string $action): bool
{
    $key = auth_account_rate_limit_key($username, $action);
    return rate_limit_check_and_record($key, 5, 300);
}

function auth_account_rate_limit_reset(string $username, string $action): void
{
    rate_limit_reset(auth_account_rate_limit_key($username, $action));
}
...
function auth_account_delete(string $username, string $activeTab = 'account-delete-account'): never
{
    ...
    $rateAction = 'delete_account';
    if (auth_account_rate_limited($username, $rateAction)) {
        flash_and_redirect('error', t('flash.auth.account_rate_limited'), $accountUrl);
    }
    ...
    auth_account_rate_limit_reset($username, $rateAction);

    if (!remember_login_revoke_all_for_user($username)) {
...
function handle_logout(string $method, array $matches): void
{
    ...
    $page = normalize_return_path(request_string($_GET, 'page'));
    if (!logout()) {
        flash_and_redirect('error', t('flash.auth.logout_failed'), $page ?: url('/'));
    }
    redirect($page ?: url('/'));
}
