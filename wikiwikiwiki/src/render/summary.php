...
{
    [$protected, $literalPlaceholders] = preserve_literal_placeholder_tokens($text);
    $placeholders = [];
    $counter = 0;
    $protected = preserve_markdown_links_and_images($protected, $placeholders, $counter);

    if ($placeholders !== []) {
        $replacements = [];
        foreach ($placeholders as $key => $token) {
            $replacements[$key] = str_starts_with($token, '![')
                ? ''
                : markdown_extract_link_label($token);
        }
        $protected = strtr($protected, $replacements);
    }

    return restore_placeholders($protected, $literalPlaceholders);
}

function markdown_extract_link_label(string $token): string
{
    $start = str_starts_with($token, '![') ? 1 : 0;
    if (!isset($token[$start]) || $token[$start] !== '[') {
        return $token;
    }
    ...
    return substr($token, $start + 1, $cursor - $start - 2);
}

function markdown_strip_reference_definitions_and_footnotes(string $text): string
{
    $definitionPattern = '/^[ \t]{0,3}\[(?:\^[^\]\r\n]+|[^\]\r\n]+)\]:[^\r\n]*(?:\R(?:[ \t]{4,}|\t).*)*/mu';
    $text = preg_replace($definitionPattern, '', $text) ?? $text;
    $text = preg_replace('/\[\^[^\]\r\n]+\]/u', '', $text) ?? $text;
    return $text;
}
...
function markdown_normalize_preview_text(string $text): string
{
    ...
    $text = markdown_strip_reference_definitions_and_footnotes($text);
