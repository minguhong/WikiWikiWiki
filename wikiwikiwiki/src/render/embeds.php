...
function render_embed_codepen(string $value, string $original): string
{
    ...
    if ($username) {
        $embedUrl = 'https://codepen.io/' . html($username) . '/embed/' . html($penId) . '?default-tab=result';
        $linkUrl = 'https://codepen.io/' . html($username) . '/pen/' . html($penId);
    } else {
        $embedUrl = 'https://codepen.io/pen/embed/' . html($penId) . '?default-tab=result';
        $linkUrl = 'https://codepen.io/pen/' . html($penId);
    }
    ...
    $caption = '<a href="' . $linkUrl . '" target="_blank" rel="noopener noreferrer">CodePen</a>';
    return render_embed_figure($iframe, $width, $caption);
}
...
function render_embed_arena(string $value, string $original): string
{
    ...
    $embedUrl = 'https://www.are.na/' . trim($path, '/') . '/embed';
    $iframeStyle = $height !== null ? ' style="height: ' . html($height) . ';"' : '';
    $iframe = '<iframe' . $iframeStyle . ' src="' . html($embedUrl) . '" loading="lazy"></iframe>';
    return render_embed_figure($iframe, $width);
}
...
function render_embed_list(string $type, string $value, string $original): string
{
    ...
    foreach ($pages as $item) {
        ...
        if ($type === 'wanted') {
            ...
            $html .= '<li><a href="' . url($title) . '" class="not-exists">' . $titleHtml . '</a> ';
            $html .= '<span aria-hidden="true">→</span> ';
            $html .= '<a href="' . url($title, '/backlinks') . '">' . html($backlinksLabel) . '</a></li>';
            continue;
        }

        $html .= '<li><a href="' . url($title) . '">' . $titleHtml . '</a></li>';
    }
    $html .= '</ul>';
    return $html;
}
