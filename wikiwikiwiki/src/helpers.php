...
function safe_random_hex(int $bytes = 8): string
{
    $size = max(1, $bytes);
    try {
        return bin2hex(random_bytes($size));
    } catch (Throwable $error) {
        static $fallbackLogged = false;
        if (!$fallbackLogged) {
            wiki_log('random.hex_fallback', [
                'bytes' => $size,
                'error' => $error->getMessage(),
            ], 'warning');
            $fallbackLogged = true;
        }
        $entropy = microtime(true) . ':' . (string) getmypid() . ':' . uniqid('', true);
        return substr(hash('sha256', $entropy), 0, $size * 2);
    }
}
...
function asset(string $file = '', bool $reset = false): string
{
    static $cache = [];
    if ($reset) {
        $cache = [];
...
function absolute_asset(string $file): string
{
    $path = asset($file);
