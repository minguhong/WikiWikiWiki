...
{
    return markdown_normalize_search_text($text);
}
...
function page_search_index_entry(string $title, ?string $content = null): array
{
    ...
    return [
        'title' => $title,
        'content' => $normalized,
        'preview' => extract_description($content, 120),
    ];
}
...
function page_search_index_normalize_cached_pages(array $cachedPages): array
{
    $index = [];
...
function page_search_index_load(bool $reset = false, bool $hydrate = true, bool $resetMeta = true): array
{
    ...
    if (!is_array($cachedPages)) {
        $locked = page_with_content_read_lock(static function (): array {
            $cachedPagesLocked = page_index_cached_pages(page_search_index_path());
            if (is_array($cachedPagesLocked)) {
                return page_search_index_normalize_cached_pages($cachedPagesLocked);
            }
        ...
        if (!is_array($locked)) {
            wiki_log('page.search_index_lock_fallback_rebuild', ['path' => page_search_index_path()], 'warning');
            $cache = page_search_index_rebuild();
            return $cache;
        }
    ...
    $cache = page_search_index_normalize_cached_pages($cachedPages);
    return $cache;
}
...
function search_snippet(string $text, string $query, int $window = 60): string
{
    ...
    return $prefix . ($marked ?? $escaped) . $suffix;
}
