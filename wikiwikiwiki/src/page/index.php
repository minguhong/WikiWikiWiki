...
function page_with_content_read_lock(callable $callback, mixed $fallback = null): mixed
{
    static $depth = 0;
    if ($depth > 0) {
        return $callback();
    ...
    try {
        ...
        return wiki_with_lock($callback, false, $fallback);
    } finally {
        $depth--;
    }
}
...
function page_index_normalize_cached_pages(array $cachedPages): array
{
    $index = [];
...
function page_index_load(bool $reset = false, bool $hydrate = true, bool $resetMeta = true): array
{
    ...
    if (!is_array($cachedPages)) {
        $locked = page_with_content_read_lock(static function (): array {
            $cachedPagesLocked = page_index_cached_pages(page_index_path());
            if (is_array($cachedPagesLocked)) {
                return page_index_normalize_cached_pages($cachedPagesLocked);
            }
        ...
        if (!is_array($locked)) {
            wiki_log('page.index_lock_fallback_rebuild', ['path' => page_index_path()], 'warning');
            $cache = page_index_rebuild();
            return $cache;
        }
    ...
    $cache = page_index_normalize_cached_pages($cachedPages);
    return $cache;
}
